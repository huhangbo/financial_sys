      ...
      <Form form={form} layout="vertical" name="form_in_modal" initialValues={info}>
        ...
        >
          <Select placeholder="选择类别">
            {categoryMap.expend != undefined && info.bill_type == 1
              ? Object.entries(categoryMap.expend).map((item: any) => {
                  return (
                    <Option key={item[0]} value={parseInt(item[0])}>
                      {item[1]}
                    </Option>
                  );
                })
  ...
  const userColumns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        return <div> {Math.floor(record.fee) / 100} </div>;
      }
    },
    {
      title: '时间',
      ...
      render: (_: any, record: any) => {
        ...
        return (
          <div>
            {' '}
            {record.year.toString() + '/' + (record.month + 1).toString() + '/' + record.day.toString()}{' '}
          </div>
        );
      }
    },
    {
      ...
      key: 'bill_id',
      render: (_: any, record: any) => {
        return <div> {categoryMap.all[record.category_id]} </div>;
      }
    },
    {
      title: '操作',
      ...
      >
        ...
        >
          <Select placeholder="请选择类别" style={{ width: '30%' }}>
            {categoryMap != undefined && billType === 1
              ? Object.entries(categoryMap.expend).map((item: any) => {
                  return (
                    <Option key={item[0]} value={parseInt(item[0])}>
                      {item[1]}
